...
export async function getDependencies(packageType: 'deb' | 'rpm', buildDir: string, applicationName: string, arch: string): Promise<string[]> {
	...
	if (JSON.stringify(sortedDependencies) !== JSON.stringify(referenceGeneratedDeps)) {
		...
		// For OSS/fork builds (e.g. GitHub Actions), the reference list may legitimately
		// differ (e.g. missing tunnel binary), so warn instead of failing the build.
		// Only Microsoft official 'stable'/'insider' Azure DevOps pipelines should
		// enforce the check; treat anything else (including fork CI) as advisory.
		const isOfficialMsBuild = !!process.env['BUILD_BUILDID'] && !!process.env['SYSTEM_TEAMPROJECT'];
		if (FAIL_BUILD_FOR_NEW_DEPENDENCIES && process.env['VSCODE_QUALITY'] !== 'oss' && isOfficialMsBuild) {
